...
class ActivitySchemeModel extends BaseModel
{
    ...
    public function saveScheme(array $payload, ?int $id = null): int
    {
        ...
        if (!in_array($categoryCode, ['penelitian', 'pengabdian', 'hilirisasi'], true)) {
            throw new InvalidArgumentException('Kategori kegiatan untuk skema tidak valid.');
        }
        if ($code === '' || $name === '') {
            throw new InvalidArgumentException('Kode dan nama skema wajib diisi.');
        }

        $pdo = db_pdo();
        if ($id !== null && $id > 0) {
            $stmt = $pdo->prepare(
                'UPDATE activity_schemes
    ...
    public function deleteScheme(int $id): bool
    {
        ...
        $pdo = db_pdo();
        $scopeStmt = $pdo->prepare('SELECT COUNT(*) FROM activity_scopes WHERE scheme_id = :scheme_id');
        $scopeStmt->execute([':scheme_id' => $id]);
        if ((int) $scopeStmt->fetchColumn() > 0) {
            throw new RuntimeException('Skema tidak dapat dihapus karena masih memiliki ruang lingkup.');
        }

        $stmt = $pdo->prepare('DELETE FROM activity_schemes WHERE id = :id');
        $stmt->execute([':id' => $id]);
    ...
    private function seedDefaults(): void
    {
        ...
        foreach (self::DEFAULT_SCHEMES as $categoryCode => $items) {
    ...
    private function slugify(string $value): string
    {
        ...
        return trim($normalized, '-') !== '' ? trim($normalized, '-') : 'scheme';
    }
}
