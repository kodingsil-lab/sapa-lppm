...
<head>
    <meta charset="UTF-8">
    <title>Surat Izin Pelaksanaan Hilirisasi</title>
    <style>
...
<body>
    ...
    <div class="meta">
        <table>
            ...
            <tr><td class="label">Perihal</td><td class="colon">:</td><td><span class="bold">Permohonan Izin Pelaksanaan Kegiatan Hilirisasi</span></td></tr>
        </table>
    </div>
    ...
    <?php
        ...
        $namaInstansiTujuan = trim((string) ($letter['institution'] ?? $namaInstansi ?? '-'));
        $jabatanPimpinan = trim((string) ($letter['destination_position'] ?? '-'));
        $tujuanLengkap = trim($jabatanPimpinan . ' ' . $namaInstansiTujuan);
        if ($tujuanLengkap === '') { $tujuanLengkap = '-'; }
        $tanggalMulaiDisplay = $formatTanggalIndo((string) $tanggalMulai);
        $tanggalSelesaiDisplay = $formatTanggalIndo((string) $tanggalSelesai);
        $nuptkPeneliti = trim((string) ($letter['applicant_nuptk'] ?? $letter['applicant_nidn'] ?? '-'));
        if ($nuptkPeneliti === '') { $nuptkPeneliti = '-'; }
        $programStudi = trim((string) ($letter['research_unit'] ?? $letter['applicant_unit'] ?? $letter['unit'] ?? '-'));
        if ($programStudi === '') { $programStudi = '-'; }
    ?>
    ...
    <p>Sehubungan dengan pelaksanaan Program Hilirisasi Riset Prioritas Pengujian Model dan Prototipe, bersama ini kami memohon izin kepada Bapak/Ibu <?= htmlspecialchars($tujuanLengkap, ENT_QUOTES, 'UTF-8'); ?> agar dapat memberikan kesempatan kepada dosen kami untuk melaksanakan kegiatan hilirisasi di instansi yang Bapak/Ibu pimpin. Adapun data pelaksana dan kegiatan hilirisasi dimaksud adalah sebagai berikut:</p>

    <table class="data">
        <tr><td class="dlabel">Nama Ketua Peneliti</td><td class="colon">:</td><td><?= htmlspecialchars((string) ($letter['researcher_name'] ?? '-'), ENT_QUOTES, 'UTF-8'); ?></td></tr>
        <tr><td class="dlabel">NUPTK</td><td class="colon">:</td><td><?= htmlspecialchars($nuptkPeneliti, ENT_QUOTES, 'UTF-8'); ?></td></tr>
        <tr><td class="dlabel">Program Studi</td><td class="colon">:</td><td><?= htmlspecialchars($programStudi, ENT_QUOTES, 'UTF-8'); ?></td></tr>
        <tr><td class="dlabel">Judul Hilirisasi</td><td class="colon">:</td><td><?= htmlspecialchars((string) ($letter['research_title'] ?? '-'), ENT_QUOTES, 'UTF-8'); ?></td></tr>
        <tr><td class="dlabel">Tahun Pelaksanaan</td><td class="colon">:</td><td><?= htmlspecialchars((string) ($letter['research_year'] ?? '-'), ENT_QUOTES, 'UTF-8'); ?></td></tr>
    ...
    <p>Sehubungan dengan hal tersebut, kami memohon kiranya Bapak/Ibu dapat memberikan izin kepada yang bersangkutan untuk melaksanakan kegiatan hilirisasi dimaksud.</p>
