...
<div class="page-content myletters-page compact-list master-list-page">
    <div class="mb-4">
        <h2 class="admin-page-title mb-1"><?= htmlspecialchars((string) ($pageTitle ?? 'Master Ruang Lingkup'), ENT_QUOTES, 'UTF-8'); ?></h2>
    ...
    <div class="card dashboard-card mt-2 letters-table-card myletters-table-card">
        <div class="card-header bg-white border-0 pt-3 px-3 d-flex justify-content-between align-items-center gap-2 flex-wrap">
            <h6 class="mb-0"><i class="bi bi-table me-2"></i>Daftar Ruang Lingkup</h6>
            <a href="<?= htmlspecialchars($basePath . '/master-data/ruang-lingkup/create', ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-primary-main master-list-add-btn">Tambah Ruang Lingkup</a>
        </div>
        <div class="card-body pt-2 pb-2">
            <div class="activity-table-wrap myletters-table-wrap table-responsive">
                <table class="table table-hover align-middle mb-0 w-100" data-custom-pagination="10">
                    <thead><tr><th>No.</th><th>Kategori</th><th>Skema</th><th>Nama Ruang Lingkup</th><th>Status</th><th class="master-action-col">Aksi</th></tr></thead>
                    <tbody>
                    <?php foreach ($items as $index => $item): ?>
                        <tr>
                            <td><?= $index + 1; ?></td>
                            <td><?= htmlspecialchars(ucfirst((string) ($item['activity_category_code'] ?? '-')), ENT_QUOTES, 'UTF-8'); ?></td>
                            <td><?= htmlspecialchars((string) ($item['scheme_name'] ?? '-'), ENT_QUOTES, 'UTF-8'); ?></td>
                            ...
                            <td class="master-action-col">
                                <div class="activity-action-wrap myletters-actions">
                                    <a href="<?= htmlspecialchars($basePath . '/master-data/ruang-lingkup/' . (int) ($item['id'] ?? 0) . '/edit', ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-sm activity-btn user-action-btn user-action-detail">Edit</a>
                                    <form method="post" action="<?= htmlspecialchars($basePath . '/master-data/ruang-lingkup/hapus', ENT_QUOTES, 'UTF-8'); ?>" class="d-inline">
                                        <input type="hidden" name="id" value="<?= (int) ($item['id'] ?? 0); ?>">
                                        <button type="submit" class="btn btn-sm activity-btn user-action-btn user-action-delete" onclick="return confirm('Hapus ruang lingkup ini?');">Hapus</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    ...
                    <?php if ($items === []): ?>
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">Belum ada data ruang lingkup.</td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
                </table>
